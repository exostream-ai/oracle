...
import type { SeedModel, SeedFamily, SeedProvider, SeedPrice, ScrapedPrice } from './worker-types.js';
...
// Track whether KV overlay has been applied (once per isolate)
export let kvPricesLoaded = false;

export function markKvPricesLoaded(): void {
  kvPricesLoaded = true;
}
...
/**
 * Patch in-memory oracle state with scraped prices.
 * Updates betaSync/betaBatch, rebuilds forward curves and ticker index.
 */
export function patchOracleWithPrices(prices: ScrapedPrice[]): void {
  if (!prices.length) return;

  // Group prices by model_id + price_type, keeping the newest observation
  const latest = new Map<string, ScrapedPrice>();
  for (const p of prices) {
    const key = `${p.model_id}:${p.price_type}`;
  ...
  // Patch each model's spot price
  for (const [key, price] of latest) {
    const [modelId, priceType] = key.split(':');
    const model = oracleState.models.get(modelId);
    if (!model) continue;

    if (priceType === 'sync') {
      model.betaSync = price.beta;
    } else if (priceType === 'batch') {
      model.betaBatch = price.beta;
    }
  }

  // Rebuild forward curves for all models with updated prices
  const tenors = ['1M', '3M', '6M'] as const;
  const tenorMonths: Record<string, number> = { '1M': 1, '3M': 3, '6M': 6 };

  for (const [modelId, model] of oracleState.models) {
    if (!model.betaSync || !model.theta) continue;

    const forwards: ForwardPrice[] = [];
    for (const tenor of tenors) {
      const t = tenorMonths[tenor];
      const decayFactor = Math.exp(-model.theta * t);
      forwards.push({
        modelId,
        priceType: 'sync',
        tenor,
    ...
    oracleState.forwardCurves.set(`${modelId}:sync`, forwards);
  }

  // Update oracle metadata
  oracleState.lastUpdate = new Date();
  oracleState.cacheAgeSeconds = 0;

  // Rebuild ticker index
  tickerIndex = new Map();
