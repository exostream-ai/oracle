-- 001_baseline.sql: Captures existing schema as migration baseline
-- ============================================================
...
CREATE TABLE IF NOT EXISTS context_tiers (
...
CREATE INDEX IF NOT EXISTS idx_spot_prices_model_time ON spot_prices (model_id, observed_at DESC);
CREATE INDEX IF NOT EXISTS idx_spot_prices_type ON spot_prices (price_type, observed_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_price_events_model ON price_events (model_id, detected_at DESC);
...
CREATE TABLE IF NOT EXISTS extrinsic_params (
...
CREATE INDEX IF NOT EXISTS idx_extrinsic_model_time ON extrinsic_params (model_id, computed_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_forward_model_tenor ON forward_prices (model_id, tenor, computed_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_scrape_provider ON scrape_log (provider_id, scraped_at DESC);
...
CREATE TABLE IF NOT EXISTS family_theta_history (
...
CREATE OR REPLACE VIEW v_current_extrinsics AS
...
CREATE OR REPLACE VIEW v_current_forwards AS
...
CREATE OR REPLACE VIEW v_greek_sheet AS
SELECT
    m.model_id,
    m.display_name,
    m.ticker_sync,
...
CREATE INDEX IF NOT EXISTS idx_spot_model_chrono ON spot_prices (model_id, observed_at ASC);

-- "What changed in the last 24 hours?"
CREATE INDEX IF NOT EXISTS idx_price_events_recent ON price_events (detected_at DESC);

-- "All active models for a provider"
CREATE INDEX IF NOT EXISTS idx_models_family ON models (family_id) WHERE status = 'active';

-- For existing databases: manually INSERT into migrations table to mark as applied
-- This baseline migration uses IF NOT EXISTS to be safe for existing schemas
